...
@admin_required
def dashboard():
    """Dashboard sekaligus halaman kelola resep (filter + paginasi + CRUD)."""
    df       = _get_df()
    ...
    return render_template('admin/dashboard.html',
        total=total_all, cats=cats, categories=categories,
        rows=rows, pages=pages, page=page, q=q, cat=cat)


# ── /recipes redirect ke dashboard ───────────────────────────────────────────

@admin_bp.route('/recipes')
@login_required
@admin_required
def recipes():
    """Redirect ke dashboard (tidak ada halaman terpisah)."""
...
def add_recipe():
    df         = _get_df()
    categories = sorted(df['Category'].unique().tolist())
    # Ambil parameter pencarian supaya bisa balik ke halaman yang sama setelah submit
    back_q    = request.values.get('back_q', '').strip()
    back_cat  = request.values.get('back_cat', '')
    back_page = request.values.get('back_page', '1')
    ...
    if request.method == 'POST':
        ...
        if errors:
            ...
            return render_template('admin/recipe_form.html',
                ...
                data=request.form,
                back_q=back_q, back_cat=back_cat, back_page=back_page)

        # Cegah submit ganda (misalnya akibat double-klik atau koneksi lambat):
        # kalau judul yang sama persis baru saja ditambahkan, jangan tambah lagi.
        if (df['Title'] == title).any():
            flash(f'Resep "{title}" sudah ada, tidak ditambahkan lagi.', 'warning')
            return redirect(url_for('admin.recipes', q=back_q, cat=back_cat, page=back_page))
